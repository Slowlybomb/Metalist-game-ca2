...
let collectibles = [
    { x: 200, y: 300, name: "Key" },
    { x: 450, y: 620, name: "Coin" },
    { x: 700, y: 100, name: "Potion" }
];
...
function init() {
    ...
    spawnEnemies(5); // Spawn n-mount enemies
...
function draw() {
    ...
    if (!player.isHidden) {
        ...
        context.fillText(player.descriptor, player.x + 18, player.y - 5);
    } else {
        context.fillStyle = 'gold';
        context.fillText("You are hidden", player.x + 18, player.y - 5);
    }
        ...
        if (!enemy.confused) {
            ...
            if (withinVision && lineOfSight && !player.isHidden) {
                if (!enemy.seePlayer) {
                    ...
                    enemy.seePlayerTime = Date.now(); // Set the time when the enemy started seeing the player
                }
            } else {
                if (enemy.seePlayer) {
                    enemy.seePlayer = false; // Player is no longer visible
                    enemy.seePlayerTime = null; // Reset the timer
                }
            }

            if (enemy.seePlayer) {
                // Enemy continues to see the player
                let timeSinceSeen = Date.now() - enemy.seePlayerTime;
                if (timeSinceSeen <= 10000) {
                    // Player has been visible within the last 10 seconds
                    moveEnemyTowardsPlayer(enemy);
                    let timeLeft = ((10000 - timeSinceSeen) / 1000).toFixed(1);
                    ...
                    enemy.seePlayerTime = null;
                }
            } else {
                // Patrol or idle behavior when not chasing the player
                patrolOrIdle(enemy);
            }
        } else {
            // Confused behavior
...
function hidePlayer() {
    ...
    let hiddenStatusElement = document.getElementById('hidden');

    if (canHide) {
        player.isHidden = true;
        hiddenStatusElement.textContent = "You are hiding";
        player.hideTimeout = setTimeout(unhidePlayer, 5000);
    } else {
        hiddenStatusElement.textContent = "No hiding spot nearby!";
        setTimeout(() => {
            if (!player.isHidden) hiddenStatusElement.textContent = "";
        }, 2000);
    }
}
